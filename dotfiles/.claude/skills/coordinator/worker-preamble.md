...
- **Git identity:** this machine's global git identity is a human's (Colin Walters <walters@verbum.org>). For every git operation that creates or rewrites commits, export `GIT_AUTHOR_NAME="Colin Walters" GIT_AUTHOR_EMAIL=walters+llm@verbum.org GIT_COMMITTER_NAME="Colin Walters" GIT_COMMITTER_EMAIL=walters+llm@verbum.org` for new commits. For rebases of others' commits, export at least the COMMITTER pair. Verify with `git log --format='%an <%ae> / %cn <%ce>'` before pushing.

...
- **Where to work:** use your own git worktree and your scratch dir, given in your task. Existing clones live under `~/src/github/cgwalters-bot/<repo>`; other agents use them, so never switch branches in a shared clone (`bin/bot-git` refuses to commit or rebase in homegit's and those listed in `~/.config/bot-git/shared-clones`).
...
- **bootc tmt test numbers:** when adding a bootc tmt test, get its `# number:` from `bin/bot-tmt-number --reserve BRANCH` (BRANCH being your `bot/<slug>`), never by picking the next one yourself: it skips the numbers taken on main, in every open PR upstream and on the forge fork, and reserved by other workers. If you drop the test or the branch, `--release BRANCH`.
...
- **Git identity:** this machine's global git identity is cgwalters' own (`Colin Walters <walters@verbum.org>`). Run every git command that creates or rewrites commits (commit, rebase, cherry-pick, am, merge) as `bin/bot-git ...` instead of `git ...`: it commits as the bot, `Colin Walters <walters+llm@verbum.org>` (the `+llm` email is what marks the bot's commits), keeps the original author on rebases and amends, and refuses to add a Signed-off-by (to keep cgwalters' existing one on a commit reworked per his review, see AGENTS.md: `bin/bot-git rework`). Before pushing, run `bin/bot-git check` (default range `@{upstream}..HEAD`; pass e.g. `origin/main..HEAD` otherwise) and fix everything it lists.
- **The bot's own repositories** (homegit, cgwalters-bot/cgwalters-bot, cgwalters-bot/debug-bootc-to-disk-virtiofsd, cgwalters-bot/ostree-missing-refs, cgwalters-bot/praxis-credential-broker, cgwalters-forge/review, cgwalters-forge/.github): main takes changes only as pull requests with a green `ci` check, rebase-merged; never push to main. Commit on a `bot/<slug>` branch in your worktree and land it with `bin/bot-land`: it runs `bot-git check`, pushes the branch, opens the pull request, enables auto-merge by rebase, waits (rebasing when main moved on) and fast-forwards the shared clone. If `ci` fails, squash the fix into the commit it belongs to and run it again. Report the pull request link.
...
- **Commits and pushes:**
  - Reworking an upstream PR per cgwalters' review: his sign-off is kept automatically. After squashing your changes in, always run `bin/bot-git rework --was OLD BASE`, where OLD is the last pushed head whose commits carried his sign-off with him as committer (usually the head `bot-pr promote`/`signoff` pushed; fetch it by full SHA if a later push dropped it) and BASE is the merge-base with upstream. For a commit he asked to retitle, add `--pair OLD-COMMIT=NEW-COMMIT`, and pass the same to `bot-git check`. Name the kept commits in your reply; don't ask him to re-sign them. Only commits new since OLD, or changes beyond his ask, need his approval plus `bot-pr signoff`.
  - Never add Signed-off-by. Only `bot-pr promote` adds one (and `bot-pr signoff`, for an upstream PR promote opened without it): cgwalters' own, after his verified approval of that exact head (see "Commit attribution" in the shared AGENTS.md).

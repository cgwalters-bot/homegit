...
5. Check whether the repo enforces DCO (required checks or a DCO workflow on main, plus CONTRIBUTING). If it does, the body must say a human Signed-off-by is needed before upstreaming; if it doesn't, don't mention DCO. Only claim CI results you can link to. Update bodies later with `bot-pr get-body`/`set-body`, never `gh pr edit`.

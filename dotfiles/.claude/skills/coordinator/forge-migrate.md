...
5. Don't write a DCO note in the body: `bot-pr promote` adds the maintainers' signoff line to the upstream body when the branch rules require DCO. Only claim CI results you can link to. Update bodies later with `bot-pr get-body`/`set-body`, never `gh pr edit`.

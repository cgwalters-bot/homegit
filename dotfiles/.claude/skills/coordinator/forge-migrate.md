...
3. Rebase the branch onto current upstream main in your own worktree (bot identity env per the worker preamble; if the branch contains commits by humans, keep their content untouched). Squash any bot "fixup"-style noise among the bot's own commits into clean logical commits. Commit messages: kernel style, why-focused, the project's trailer policy (Generated-by: AI by default).
...
5. Check whether the repo enforces DCO. Decide that from the branch rules API and workflows as upstream-pr/SKILL.md describes (`gh api repos/O/R/rules/branches/<default>`: a required status check with a DCO context), never from CONTRIBUTING text. If it does, the body must say that a maintainer has to sign off before merging; if it doesn't, don't mention DCO. Only claim CI results you can link to. Update bodies later with `bot-pr get-body`/`set-body`, never `gh pr edit`.

...
- **rebase** (optional): `rebase: conflicts-only` when the maintainers asked the bot to rebase its PRs only to resolve conflicts, with a link to the ask in the body. bot-watch and `bot-pr rebase` then skip conflict-free rebases there, as they do on their own for repositories with a merge queue (a `merge_queue` rule, or a workflow on `merge_group`). `rebase: any` overrides that detection, for repositories cgwalters maintains where he wants PRs that are merely behind rebased so CI passes (bootc-dev/bootc and its neighbours); say so in the body. Only cgwalters' wish sets `any`, never on repositories he doesn't maintain, and it counts only once pushed to homegit's main. Leave it out otherwise; a record sets it once, so it can't say both.
